...
As shown from the sequence diagram above (Refer to Figure 33), the method `handleChangeThemeEvent()` in `MainWindow` will handle the event and change the theme of iContacts through the method `changeTheme()` accordingly.
