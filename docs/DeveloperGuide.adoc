...
=== Toggling mechanism
The toggling mechanism is an event-driven mechanism. It is facilitated by events `BrowserAndRemindersPanelToggleEvent`, `TurnLabelsOffEvent` and `TurnLabelsOnEvent`. +

image::togglingMechanism1.png[width="790"]
_Figure 1: High-level overview of the toggling mechanism._

As seen from the diagram above, when the user type in the `toggle` command and the `Logic` component execute the command, the `BrowserAndRemindersPanelToggleEvent` will be posted. +
The following code snippet is taken from the `Toggle` class:
...
_Figure 2: Lower-level overview of the toggling mechanism._
...
The following code snippet is taken from the `BrowserAndRemindersPanel` class:
...
reminders should be turned off, and when the reminders panel is brought to the front, these heading should be turned on. This can
be seen from the following code snippet, taken from the `HeaderPane` class:
...
This can be seen from the following code snippet taken from the `BrowserAndRemindersPanel` class:
