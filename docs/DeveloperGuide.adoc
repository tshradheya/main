...
The `Circle` shape is used for better UI/UX design and is filled with the `ImagePattern`. The wireframe used to display the image for each person is shown below:
...
The way these UI elements are added is shown in the following diagram:
...
The following is the sequence diagram which shows how the event is triggered and how the saving of updated address book takes place:
...
The figure below (Refer to figure TBD) is the wireframe for the `DetailsPanel`:
...
The command is added into the `Logic` component according to following class diagram:
