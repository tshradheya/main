...
_Figure 28: UML diagram showing the implementation of Reminder._

The `Reminder` object represents a reminder in iContacts. It contains a `String` variable, which represents
the actual reminder from the user, and also a `Date`, `Time` and `Status` object which represents the date, time and link:#status[status] of the reminder respectively (Refer to Figure 28). +

In terms of how `Reminder` objects are kept in-memory during the execution of the program, `Reminder` objects are
kept within `UniqueReminderList` (Refer to Figure 28), which assures that there are no duplicate `Reminder` objects. The `UniqueReminderList`
object is then kept and used by `ModelManager` to carry out commands related to reminders while the program is running. +

As can be seen from Figure 28 above, as part of _defensive programming_, `Reminder` implements the `ReadOnlyReminder` interface. The `ReadOnlyReminder` interface only allows read operations, thus helping
to prevent inappropriate modifications to `Reminder` objects. Similarly, `UniqueReminderList` implements the `ReadOnlyUniqueReminderList` interface for the same reason. +
...
As can be seen from the code snippet above and in Figure 28, as part of _defensive programming_, `XmlSerializableReminders` also implements the `ReadOnlyUniqueReminderList` interface. +
...
and then the `saveDataToFile` method in `XmlUtil`.
...
The theme-changing mechanism is an event-driven mechanism. The above diagram (Refer to Figure 29) shows the high-level overview of the component interactions for the theme-changing mechanism.
...
As shown from the sequence diagram above (Refer to Figure 31), the method `handleChangeThemeEvent()` in `MainWindow` will handle the event and change the theme of the address book through the method `changeTheme()` accordingly.
// end::theme[]
...
_Figure 32 : Saving documentation as PDF files in Chrome_
...
. Use different colors for different tags inside person cards. For example, `friends` tags can be all in grey, and `colleagues` tags can be all in red (Refer to Figures 33 and 34).
+
**Before**
+
image::getting-started-ui-tag-before.png[width="300"]
_Figure 33: Before applying colors to tags._
+
**After**
+
image::getting-started-ui-tag-after.png[width="300"]
_Figure 34: After applying colors to tags._
+
****
* Hints
...
_Figure 35: Style before modification._
+
**After**
+
image::getting-started-ui-result-after.png[width="200"]
_Figure 36: Style after modification._
+
****
* Hints
...
. Modify the link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to show the total number of people in the address book (Refer to Figures 37 and 38).
+
**Before**
+
image::getting-started-ui-status-before.png[width="500"]
_Figure 37: No values showed before modification._
+
**After**
+
image::getting-started-ui-status-after.png[width="500"]
_Figure 38: Number of persons showed after modification._
+
****
* Hints
