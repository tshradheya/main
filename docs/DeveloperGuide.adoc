...
|`* *` |user |I want to be able to add Display Picture for my contact |so that I can easily identify them
...
|`*` |busy user |I should be able to create meeting reminders with my link:#contacts[contacts] |so that I don't forget whom should I be meeting at what time
...
(For all use cases below, the *System* is the `iContacts` and the *Actor* is the `user`, unless specified otherwise)
...
3.  iContacts creates file with the link:#contact-information[contact information] of specified contacts
4.  User specifies the location where link:#data-file[data file] needs to be stored
5.  iContacts saves the data file in proper format and indicates success
...
+
[none]
** 4a1. iContacts shows an error message and doesn't merge the duplicate ones
+
Use case resumes at step 1.
...
2.  iContacts finds list of all people satisfying all attributes
3.  iContacts displays list of all such contacts
