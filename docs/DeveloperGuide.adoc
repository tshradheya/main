...
The method `getRangefromInput()` splits the String range using a separator and returns a String array for the different values in the String range.
...
The `ImportCommand` first checks if an AddressBook object can be initialise from the specified path, and if it does it initialises a new Person Object and adds it into the main AddressBook object through the model. Below is the code snippet:
[source,java]
----
public class ExportCommand extends Command {
...
**Pros:** Does not affect the users contact's PopularityCounter and allows the user to set their own preferred DisplayPicture of a contact  +
**Cons:** Users wont be able to share display picture even if they want to. +
**Alternative 2:** Import the contacts as how it was exported+
...
The `SortCommand` uses a `SortedList` that is initialise with the current filteredPerson and is sorted using a comparator. Below is the code snippet:
