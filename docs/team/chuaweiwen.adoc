...
* Since the nickname is just another field for the contact, it would make much more sense to add and edit the nickname using the `add` and `edit` command respectively without the `nickname` command.
